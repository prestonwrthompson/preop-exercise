...
def _format_bp_exclusion_details(vital: object) -> str:
    vital_date = getattr(vital, "date", None) or "unknown date"
    vital_source = getattr(vital, "source", None) or "unknown source"
    systolic = getattr(vital, "systolic", None)
    diastolic = getattr(vital, "diastolic", None)
    return (
        f"latest BP on {vital_date} (source: {vital_source}) "
        f"systolic={systolic}, diastolic={diastolic}; "
        "threshold systolic>=180 or diastolic>=110"
    )


def _summarize_documents(submission: PatientSubmission, limit: int = 4) -> str:
    documents = submission.documents or []
    if not documents:
        return "none"
    parts: list[str] = []
    for document in documents[:limit]:
        doc_type = document.type or "unknown type"
        doc_date = document.date or "no date"
        parts.append(f"{doc_type} ({doc_date})")
    return "; ".join(parts)


def _summarize_labs(submission: PatientSubmission, limit: int = 4) -> str:
    labs = submission.labs or []
    if not labs:
        return "none"
    parts: list[str] = []
    for lab in labs[:limit]:
        code = lab.code or lab.display or "unknown lab"
        effective_at = lab.effective_at or "no effective_at"
        parts.append(f"{code} (effective_at {effective_at})")
    return "; ".join(parts)


def _find_any_lab_index(submission: PatientSubmission, test: str) -> int | None:
    for index, lab in enumerate(submission.labs or []):
        if _lab_code_matches(lab.code, test):
            return index
    return None


def _missing_lab_issue(
    ...
    *,
    ...
    procedure_date = procedure.procedure_date or "unknown"

    any_index = _find_any_lab_index(submission, test)
    if any_index is not None:
        lab = submission.labs[any_index]
        return _issue(
            "REQUIRED_TESTING",
            f"Missing required {test}",
            f"labs[{any_index}]",
            (
                f"No {test} result with valid effective_at for procedure_date "
                f"{procedure_date} (procedure_risk {risk}); "
                f"found {lab.code or test} effective_at={lab.effective_at!s}"
            ),
        )
    ...
    if labs:
        return _issue(
            ...
            (
                f"No {test} result with valid effective_at for procedure_date "
                f"{procedure_date} (procedure_risk {risk}); "
                f"labs present: {_summarize_labs(submission)}"
            ),
        )
    ...
    return _issue(
        ...
        (
            f"No {test} result with valid effective_at for procedure_date "
            f"{procedure_date} (procedure_risk {risk}); no labs in submission"
        ),
    )


def _format_anticoag_details(
    submission: PatientSubmission,
    med_index: int,
    plan_doc_index: int | None,
) -> str:
    medication = submission.medications[med_index]
    med_name = medication.name or "unknown medication"
    details = (
        f"Active anticoagulant {med_name} (medications[{med_index}]) "
    ...
    document = documents[plan_doc_index]
    snippet = _truncate_text(document.text or "")
    if snippet:
        details += f"; plan document documents[{plan_doc_index}] ({document.type}): {snippet}"
    elif document.type:
        details += f"; plan document documents[{plan_doc_index}] type={document.type}"
    return details
            ...
            if systolic >= 180 or diastolic >= 110:
                issues.append(
                    _issue(
                        ...
                        f"vitals[{bp_index}]",
                        _format_bp_exclusion_details(vital),
                    )
                )
...
def _evaluate_required_testing(submission: PatientSubmission) -> list[TriageIssue]:
    ...
    if cbc_index is None:
        issues.append(_missing_lab_issue(submission, test="CBC", risk=risk))
    else:
    ...
    if risk == "HIGH":
        ...
        if cmp_index is None:
            issues.append(_missing_lab_issue(submission, test="CMP", risk=risk))
        else:
            ...
            issues.append(
                _issue(
                    ...
                    (
                        ...
                        f"documents present: {_summarize_documents(submission)}"
                    ),
                )
            )
...
def _evaluate_anticoagulation_from_extraction(
    ...
    issues.append(
        _issue(
            ...
            f"medications[{active_anticoag_index}]",
            _format_anticoag_details(
                submission, active_anticoag_index, plan_doc_index
            ),
        )
    )
    return issues
